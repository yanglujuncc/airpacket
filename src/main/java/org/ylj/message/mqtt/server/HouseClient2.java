...
public class HouseClient2 {
	...
	public static void main(String[] args) throws Exception {
		...
		connection.publish("clients/" + replyIMMsg.to + "/up", replyCoreJsonMsgJsonString.getBytes(), QoS.AT_LEAST_ONCE,
				false);
		...
		/*
		 * 
		 * Topic[] topics = {new Topic("foo", QoS.AT_LEAST_ONCE)}; byte[] qoses
		 * = connection.subscribe(topics); Message message =
		 * connection.receive(); System.out.println(message.getTopic()); byte[]
		 * payload = message.getPayload(); // process the message then:
		 * message.ack();
		 */
