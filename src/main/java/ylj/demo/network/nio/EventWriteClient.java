 ...
 *
 */
public class EventWriteClient implements Runnable {

	 private static final Logger logger = Logger.getLogger(EventWriteClient.class);
	...
	public void addWriteTask(ByteBuffer toWriteByteBuffer,boolean toWrite) throws ClosedChannelException{
		writeLock.lock();
		try {   
	...
	private void  doWrite() throws IOException{
		...
		try {   
			while(toWriteQueue.size()>0){
				...
				int writen = socketChannel.write(toWriteBuffer); // just write to the socket buffer,
				totalWriten+=writen;
	...
	*/
	...
	public static void main(String[] args) throws ClosedChannelException, IOException, InterruptedException {
		...
		EventWriteClient client=new EventWriteClient();
		for(int i=0;i<10;i++){
			client.addWriteTask(nextWriteBuffer2(1024*1024),false);
		}
